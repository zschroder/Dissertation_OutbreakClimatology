...
```{r}
max(BigDays1994_2020$minCIN)
min(BigDays1994_2020$minCIN)
mean(BigDays1994_2020$minCIN)

test <- BigDays1994_2020 %>% 
  filter(maxHLCY <= 2000)
...
```{r}
Top3 <- BigDays1994_2020 %>%
  arrange(desc(nT)) %>%
  head(3)

Bot3 <- BigDays1994_2020 %>%
  arrange(desc(nT)) %>%
  tail(3)
```
...
**Bulk Shear**
```{r}
SLBS_May20 <- crop(SLBS, extent(May20))
SLBS_extent<- mask(SLBS_May20, May20)
plot(SLBS_extent)
...
```{r}
( A <- ggplot(BigDays1994_2020, (aes(maxCAPE))) +
...
```{r}
( B <- ggplot(BigDays1994_2020, (aes(minCIN))) +
  #geom_smooth(aes(x = week, y = probs/100), span = .5, se = FALSE, color = "gray70", size = 1) +
...
```{r}
( C <- ggplot(BigDays1994_2020, (aes(maxBS_shallow))) +
  #geom_smooth(aes(x = week, y = probs/100), span = .5, se = FALSE, color = "gray70", size = 1) +
...
```{r}
( D <- ggplot(BigDays1994_2020, (aes(maxBS_deep))) +
  #geom_smooth(aes(x = week, y = probs/100), span = .5, se = FALSE, color = "gray70", size = 1) +
...
```{r}
( E <- ggplot(BigDays1994_2020, (aes(maxHLCY))) +
  #geom_smooth(aes(x = week, y = probs/100), span = .5, se = FALSE, color = "gray70", size = 1) +
...
#####################################################
Sensitivity to cluster analysis
...
library(raster)
  ...
  rb <- brick(paste0("E:/Zoe/Projects/Torn_Freq/merged_AWIP32.19990606/merged_AWIP32.1999060615"))
  CAPE <- raster(rb, layer = 375)
  HLCY <- raster(rb, layer = 323)
  CIN <- raster(rb, layer = 376)
  UGRD500 <- raster(rb, layer = 117) 
