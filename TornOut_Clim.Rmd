...
totcas <- Torn2018 %>%
  group_by(st) %>%
  summarize(casualties = sum(cas))

totcas <- st_transform(totcas, crs = st_crs(stateBorders))

usa <- c("DE", "ME", "NJ", "VT")
miss_usa <- states.sf %>%
  filter(stusps %in% usa)

casualties <- as.data.frame(as.matrix(c(0,0,0,0), nrow = 1))
colnames(casualties) = c("casualties")

miss_usa <- miss_usa %>%
  mutate(casualties = rep(0,4))

miss_usa <- st_transform(miss_usa, crs = st_crs(stateBorders))

miss_usa <- miss_usa %>%
 select(stusps, casualties, geometry)
colnames(miss_usa) <- c("st", "casualties", "geometry")

totcas <- rbind(totcas, miss_usa)
states.sf <- st_transform(states.sf, 
                            crs = st_crs(stateBorders))

states.sf = cbind(states.sf,totcas$casualties)
```
...
tm_shape(states.sf, is.master = TRUE, projection = "merc") +
  tm_fill("totcas.casualties",
            title = "Tornado Counts",
            palette = "Reds", breaks = c(0, 5, 10, 15, 20, 25, 30, 35)) +
